...
void handle_click(char* label) {
    if (label[0] >= '0' && label[0] <= '9') {
        ...
        stored_val = current_val; current_op = 1; new_input = 1;
    } else if (strcmp(label, "-") == 0) {
        stored_val = current_val; current_op = 2; new_input = 1;
    } else if (strcmp(label, "x") == 0) {
        stored_val = current_val; current_op = 3; new_input = 1;
    } else if (strcmp(label, "/") == 0) {
        stored_val = current_val; current_op = 4; new_input = 1;
    } else if (strcmp(label, "=") == 0) {
...
int main() {
    ...
    while (!win->closed) {
        ...
        int text_w = gui_get_string_width(display_str, strlen(display_str));
        int text_x = win->w - text_w - 15;
        int text_y = 80 - gui_get_font_height() - 10;
        gui_draw_string(win, text_x, text_y, display_str, C_DISP_TEXT);
        ...
        for (int i = 0; i < sizeof(buttons) / sizeof(CalcBtn); i++) {
            ...
            int label_w = gui_get_string_width(b.label, strlen(b.label));
            int label_x = b.x + (b.w - label_w) / 2;
            int label_y = b.y + (b.h - gui_get_font_height()) / 2;
            
            gui_draw_string(win, label_x, label_y, b.label, C_DISP_TEXT);
