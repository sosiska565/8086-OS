...
void term_putchar(char c, uint32_t fg) {
    if (ansi_state == 1) {
        ...
        if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
            ansi_buf[ansi_pos] = '\0';
            if (c == 'm') {
                if (ansi_pos == 0) { term_fg_color = C_TEXT; term_bg_color = C_BG; }
                else {
                    char* p = ansi_buf;
                    while(*p) {
                        int code = atoi(p);
...
void render_terminal(gui_window_t* win) {
    ...
    if (scroll_offset == 0 && active_child_pid == 0) {
        ...
        if (screen_cy >= 0 && screen_cy < term_rows) {
            gui_draw_rect(win, cursor_x * char_w, screen_cy * char_h + (char_h - 2), char_w, 2, C_TEXT);
        }
    }
...
void draw_prompt() {
    ...
    term_print("\n", 0);
    term_print("\033[92m", 0); 
    term_print("root", 0);
    term_print("\033[97m", 0); 
    term_print("@8086-os ", 0);
    term_print("\033[94m", 0); 
    term_print(cwd, 0);
    term_print("\033[97m", 0); 
    term_print("\n$ ", 0);
    term_print("\033[0m", 0);  
...
void update_input_line() {
    int cy = prompt_start_y;
    int cx = prompt_start_x;
    while(cy <= abs_cursor_y) {
        GRID(cy, cx).c = ' ';
        cx++;
        if (cx >= term_cols) { cx = 0; cy++; }
    }
    abs_cursor_y = prompt_start_y;
...
int resolve_path(char* cmd, char* resolved) {
    ...
    if(has_slash) {
        if(get_file_size(cmd) > 0) { strcpy(resolved, cmd); return 1; }
        char full_path[256]; sprintf(full_path, "%s.elf", cmd);
...
void execute_command(gui_window_t* win) {
    ...
    if(strcmp(cmd, "clear") == 0) {
        ...
        if(argc > 1) {
            if(chdir(args[1]) != 0) term_print("cd: No such directory\n", C_ERROR);
        }
    } else if(strcmp(cmd, "pwd") == 0) {
        char c[256]; getcwd(c, 256); term_print(c, 0); term_print("\n", 0);
    } else if(strcmp(cmd, "rehash") == 0) {
        rehash_path(); term_print("Path refreshed.\n", 0);
        ...
        if(resolve_path(cmd, resolved)) {
            ...
            for(int i=0; i<argc; i++) {
                if(strcmp(args[i], ">") == 0) {
                    args[i] = NULL;
                    if(i+1 < argc) redirect_file = args[i+1];
                    break;
                }
                if(strcmp(args[i], "|") == 0) {
                    args[i] = NULL;
                ...
                while(active_child_pid > 0) {
                    ...
                    while(gui_poll_event(win, &ev)) {
                        if(ev.type == GUI_EV_CLOSE) {
                            kill(main_pid, 9);
                            shm_get(1001, 0); free(term_grid);
                            gui_destroy_window(win); exit(0);
                        }
...
int main() {
    ...
    gui_window_t* win = gui_create_window("Terminal", 640, 400);
    if(!win) return 1;
    gui_set_resizable(win, 1);
    
    char_w = gui_get_font_width();
    if (char_w <= 0) char_w = 8;
    char_h = gui_get_font_height() + 2; 
    ...
    while(running && !win->closed) {
        ...
        while(gui_poll_event(win, &ev)) {
            if(ev.type == GUI_EV_CLOSE) running = 0;
                ...
                if ((mods & 2) && (c == '=' || c == '+')) {
                    term_font_size += 2.0f;
                    gui_set_font_size(term_font_size);
