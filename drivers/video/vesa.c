...
void put_pixel(int x, int y, uint32_t color, int tar) {
    uint8_t *target = (tar != 0) ? (uint8_t*)back_buffer : (uint8_t*)video_memory;
    if (target == 0) target = (uint8_t*)video_memory;
    if (x < 0 || x >= screen_width || y < 0 || y >= screen_height) return;
    uint8_t *pixel_addr = target + (y * screen_pitch) + (x * (screen_bpp / 8));
    if (screen_bpp == 32) *(uint32_t*)pixel_addr = color;
    else if (screen_bpp == 24) { pixel_addr[0] = color & 0xFF; pixel_addr[1] = (color >> 8) & 0xFF; pixel_addr[2] = (color >> 16) & 0xFF; }
}

void clear_screen_vesa(uint32_t color) {
    if (back_buffer == 0) return;
    if (color == 0) fast_memset(back_buffer, 0, buffer_size_bytes / 4);
    else {
        if (screen_bpp == 32) fast_memset(back_buffer, color, buffer_size_bytes / 4);
        else { for(int y = 0; y < screen_height; y++) for(int x = 0; x < screen_width; x++) put_pixel(x, y, color, 1); }
    }
    vesa_render_buffer();
}

void vesa_render_buffer() {
    if (video_memory && back_buffer) fast_memcpy(video_memory, back_buffer, buffer_size_bytes);
}
...
void vesa_render_rect(int x, int y, int w, int h) {
    ...
    for (int i = 0; i < h; i++) {
        ...
        src_start += screen_pitch; dst_start += screen_pitch;
    }
}
