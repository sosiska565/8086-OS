...
typedef struct Window{
    int id; int x, y; int width, height;
    uint32_t bg_color; uint32_t border_color;
    int cursor_x, cursor_y; uint32_t text_color;
    int workspace; int is_fullscreen;
    int stretch_x; int stretch_y; int anim_scale; 
    int blur; 

    struct Window *next;
    unsigned int *char_buffer; uint32_t *color_buffer;
    int rows, cols;
