...
static int safe_strlen(char *str, int max_len) {
    if (!validate_user_ptr(str, 1)) return -1;
    int len = 0;
    while (len < max_len) {
        if (!validate_user_ptr(str + len, 1)) return -1;
...
static void sys_read(struct syscall_registers *regs) { 
    ...
    regs->eax = sys_read_fd((int)regs->ebx, (uint8_t*)regs->ecx, (uint32_t)regs->edx); 
}
...
static void sys_write_file(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 256) < 0 || !validate_user_ptr((void*)regs->ecx, regs->edx)) { regs->eax = -1; return; }
    char abs_path[256]; get_absolute_path(current_task->cwd, (char*)regs->ebx, abs_path); 
    if (current_task->uid != 0 && (strcmp(abs_path, "/kernel.cfg") == 0 || strncmp(abs_path, "/path", 5) == 0)) { regs->eax = -1; return; } 
    regs->eax = vfs_write(abs_path, (uint8_t*)regs->ecx, (uint32_t)regs->edx); 
}

static void sys_delete_file(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 256) < 0) { regs->eax = -1; return; }
    char abs_path[256]; get_absolute_path(current_task->cwd, (char*)regs->ebx, abs_path); 
    if (current_task->uid != 0 && (strcmp(abs_path, "/kernel.cfg") == 0 || strncmp(abs_path, "/path", 5) == 0)) { regs->eax = -1; return; } 
...
static void sys_spawn(struct syscall_registers *regs) { 
    ...
    if (redirect && safe_strlen(redirect, 256) >= 0 && redirect[0] != '\0') { 
        char r_path[256]; get_absolute_path(current_task->cwd, redirect, r_path); 
        int fd = sys_open(r_path, 1 | 0x0200 | 0x0400); 
        if(fd != -1) { regs->eax = spawn_process_ext(abs_path, (char**)regs->ecx, -1, fd); sys_close(fd); } 
        else { regs->eax = -1; } 
    } else { 
        regs->eax = spawn_process(abs_path, (char**)regs->ecx); 
    } 
}

static void sys_spawn_ext(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 256) < 0) { regs->eax = -1; return; }
    char abs_path[256]; get_absolute_path(current_task->cwd, (char*)regs->ebx, abs_path); 
    regs->eax = spawn_process_ext(abs_path, (char**)regs->ecx, (int)regs->edx, (int)regs->esi); 
}
...
static void sys_read_file(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 256) < 0 || !validate_user_ptr((void*)regs->ecx, 1)) { regs->eax = -1; return; }
    char abs_path[256]; get_absolute_path(current_task->cwd, (char*)regs->ebx, abs_path); 
    regs->eax = vfs_read(abs_path, (uint8_t*)regs->ecx); 
}

static void sys_get_file_size(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 256) < 0) { regs->eax = -1; return; }
    char abs_path[256]; get_absolute_path(current_task->cwd, (char*)regs->ebx, abs_path); 
    regs->eax = vfs_get_size(abs_path); 
}
...
static void sys_get_cursor(struct syscall_registers *regs) { 
    if (validate_user_ptr((void*)regs->ebx, 4) && validate_user_ptr((void*)regs->ecx, 4))
        gfx_get_cursor((int*)regs->ebx, (int*)regs->ecx); 
}
...
static void sys_get_term_size(struct syscall_registers *regs) { 
    if (regs->ebx && validate_user_ptr((void*)regs->ebx, 4)) *(int*)regs->ebx = term_cols;
    if (regs->ecx && validate_user_ptr((void*)regs->ecx, 4)) *(int*)regs->ecx = term_rows;
    regs->eax = 0; 
}

static void sys_print(struct syscall_registers *regs) { 
    char* str = (char*)regs->ebx; 
    int len = safe_strlen(str, 8192);
    if (len < 0) { regs->eax = 0; return; }
    for (int i = 0; i < len; i++) gfx_putc(str[i]);
    regs->eax = len; 
}
...
static void sys_get_tasks(struct syscall_registers *regs) { 
    if (!validate_user_ptr((void*)regs->ebx, regs->ecx * sizeof(task_info_user_t))) { regs->eax = 0; return; }
    task_info_user_t* buf = (task_info_user_t*)regs->ebx; int max_count = (int)regs->ecx; int count = 0; 
    if (!ready_queue) { regs->eax = 0; return; } 
    Task *t = ready_queue; 
    do { 
        if (t->state != TASK_DEAD && count < max_count) { 
            buf[count].id = t->id; buf[count].parent_id = t->parent_id; buf[count].state = t->state; strcpy(buf[count].name, t->name); count++; 
        } 
        t = t->next; 
    } while (t != ready_queue); 
    regs->eax = count; 
...
static void sys_get_mem_info(struct syscall_registers *regs) { 
    if (validate_user_ptr((void*)regs->ebx, 4)) *(uint32_t*)regs->ebx = get_used_memory(); 
    if (validate_user_ptr((void*)regs->ecx, 4)) *(uint32_t*)regs->ecx = get_total_memory(); 
}

extern Config *global_cfg;
static void sys_getenv(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 64) < 0 || !validate_user_ptr((void*)regs->ecx, 128)) { regs->eax = 0; return; }
...
static void sys_fork(struct syscall_registers *regs) { 
    if (!validate_user_ptr((void*)regs->ebx, sizeof(process_struct))) { regs->eax = -1; return; }
    process_struct *p = (process_struct*)regs->ebx; 
    regs->eax = create_process((void (*)(int, char**))p->foo, p->argc, (char**)p->argv, (char*)p->name, current_task->page_dir, -1, -1); 
}
...
static void sys_kill(struct syscall_registers *regs) { 
    ...
    send_signal(regs->ebx, sig); regs->eax = 0;
}
...
static void sys_chdir(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 256) < 0) { regs->eax = -1; return; }
    char abs_path[256]; get_absolute_path(current_task->cwd, (char*)regs->ebx, abs_path); 
    uint8_t type; 
    if (vfs_get_attr(abs_path, &type) && type == VFS_ATTR_DIR) { strcpy(current_task->cwd, abs_path); regs->eax = 0; } else regs->eax = -1; 
}

static void sys_readdir(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 256) < 0 || !validate_user_ptr((void*)regs->edx, sizeof(vfs_dirent_t))) { regs->eax = 0; return; }
    char abs_path[256]; get_absolute_path(current_task->cwd, (char*)regs->ebx, abs_path); 
    regs->eax = vfs_readdir(abs_path, (int)regs->ecx, (vfs_dirent_t*)regs->edx); 
}

static void sys_mkdir(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 256) < 0) { regs->eax = -1; return; }
    char abs_path[256]; get_absolute_path(current_task->cwd, (char*)regs->ebx, abs_path); 
    regs->eax = vfs_mkdir(abs_path); 
}

static void sys_getcwd(struct syscall_registers *regs) { 
    if (!validate_user_ptr((void*)regs->ebx, 256)) { regs->eax = -1; return; }
    strcpy((char*)regs->ebx, current_task->cwd); regs->eax = 0; 
}

static void sys_dlopen(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 128) < 0) { regs->eax = 0; return; }
    regs->eax = load_library(current_task, (char*)regs->ebx); 
}

static void sys_dlsym(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ecx, 64) < 0) { regs->eax = 0; return; }
    regs->eax = get_symbol(current_task, regs->ebx, (char*)regs->ecx); 
}

static void sys_mount(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 64) < 0 || safe_strlen((char*)regs->ecx, 64) < 0 || safe_strlen((char*)regs->edx, 16) < 0) { regs->eax = -1; return; }
    regs->eax = vfs_mount((char*)regs->ebx, (char*)regs->ecx, (char*)regs->edx); 
}

static void sys_unmount(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 64) < 0) { regs->eax = -1; return; }
    regs->eax = vfs_unmount((char*)regs->ebx); 
}

static void sys_detach(struct syscall_registers *regs) { 
    int target_pid = (int)regs->ebx;
    if (target_pid <= 0) { current_task->parent_id = 1; } 
    else {
        if (ready_queue) {
            Task *t = ready_queue;
            do { if (t->id == target_pid) { t->parent_id = 1; break; } t = t->next; } while (t != ready_queue);
        }
    }
    regs->eax = 0; 
}
...
static void sys_get_screen_info(struct syscall_registers *regs) { 
    if (regs->ebx && validate_user_ptr((void*)regs->ebx, 4)) *(int*)regs->ebx = screen_width; 
    if (regs->ecx && validate_user_ptr((void*)regs->ecx, 4)) *(int*)regs->ecx = screen_height; 
    if (regs->edx && validate_user_ptr((void*)regs->edx, 4)) *(int*)regs->edx = screen_bpp; 
    regs->eax = 0; 
}
...
static void sys_flush_screen(struct syscall_registers *regs) { 
    ...
    if (user_buffer && validate_user_ptr(user_buffer, screen_height * screen_pitch) && video_memory) { 
...
static void wrap_open(struct syscall_registers *regs) { 
    if (safe_strlen((char*)regs->ebx, 256) < 0) { regs->eax = -1; return; }
    regs->eax = sys_open((char*)regs->ebx, regs->ecx); 
}
...
static void sys_pipe(struct syscall_registers *regs) { 
    if (!validate_user_ptr((void*)regs->ebx, 8)) { regs->eax = -1; return; }
    regs->eax = sys_pipe_impl((int*)regs->ebx); 
}
...
static void sys_shm_get(struct syscall_registers *regs) {
    ...
    if (size == 0xFFFFFFFF) { 
        for (int i = 0; i < MAX_SHM; i++) { if (shm_table[i].key == key && shm_table[i].phys_addr) { regs->eax = 1; return; } }
        regs->eax = 0; return;
    }
    if (size == 0) { 
        for (int i = 0; i < MAX_SHM; i++) {
            if (shm_table[i].key == key && shm_table[i].phys_addr) {
                ...
                shm_table[i].phys_addr = 0; shm_table[i].key = 0; shm_table[i].size = 0; shm_table[i].owner_pid = 0;
                regs->eax = 0; return;
            }
        }
        regs->eax = -1; return;
...
void net_close_tcp_socket(int pcb_id) {
    ...
    if (pcb) { tcp_arg(pcb, NULL); tcp_recv(pcb, NULL); tcp_err(pcb, NULL); tcp_poll(pcb, NULL, 0); tcp_sent(pcb, NULL); tcp_abort(pcb); }
}
...
static err_t tcp_recv_callback(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err) {
    ...
    if (kf->in_use && kf->buffer != NULL) {
        if (kf->size + p->tot_len > kf->capacity) return ERR_MEM; 
        pbuf_copy_partial(p, kf->buffer + kf->size, p->tot_len, 0);
        kf->size += p->tot_len; kf->is_dirty = 1;
    }
    pbuf_free(p); return ERR_OK;
}

static err_t tcp_connected_callback(void *arg, struct tcp_pcb *tpcb, err_t err) {
    int sys_fd = (int)arg; system_open_files[sys_fd].is_dirty = 1; return ERR_OK;
}

static void sys_socket(struct syscall_registers *regs) {
    int type = (int)regs->ecx; int proto = (int)regs->edx;
    int free_fd = -1;
    ...
    system_open_files[sys_fd].in_use = 1; system_open_files[sys_fd].refcount = 1;
    system_open_files[sys_fd].capacity = 65536; system_open_files[sys_fd].buffer = kmalloc(65536);
    system_open_files[sys_fd].is_dirty = 0; system_open_files[sys_fd].size = 0; system_open_files[sys_fd].offset = 0;

    if (type == 3) { 
        struct raw_pcb *pcb = raw_new(proto); system_open_files[sys_fd].type = FILE_TYPE_SOCKET;
        system_open_files[sys_fd].socket_id = (int)pcb; raw_recv(pcb, raw_net_recv, (void*)sys_fd);
    } else if (type == 1) { 
        struct tcp_pcb *pcb = tcp_new(); system_open_files[sys_fd].type = FILE_TYPE_SOCKET_TCP;
        system_open_files[sys_fd].socket_id = (int)pcb; tcp_arg(pcb, (void*)sys_fd); tcp_recv(pcb, tcp_recv_callback);
    } else {
        system_open_files[sys_fd].in_use = 0; kfree(system_open_files[sys_fd].buffer); regs->eax = -1; return;
    }
    current_task->fd_table[free_fd] = sys_fd; regs->eax = free_fd;
}
...
static void sys_connect(struct syscall_registers *regs) {
    if (!validate_user_ptr((void*)regs->ecx, sizeof(struct _sys_sockaddr_in))) { regs->eax = -1; return; }
    int fd = (int)regs->ebx; struct _sys_sockaddr_in *addr = (struct _sys_sockaddr_in *)regs->ecx;
    if (fd < 0 || fd >= MAX_FDS) { regs->eax = -1; return; }
    int sys_fd = current_task->fd_table[fd]; if (sys_fd == -1) { regs->eax = -1; return; }

    ip_addr_t ip; ip.addr = addr->s_addr; KFile *kf = &system_open_files[sys_fd];
    if (kf->type == FILE_TYPE_SOCKET) { raw_connect((struct raw_pcb *)kf->socket_id, &ip); regs->eax = 0; } 
    else if (kf->type == FILE_TYPE_SOCKET_TCP) {
        if (kf->offset == 0) { 
            kf->offset = 1; kf->is_dirty = 0;
            tcp_connect((struct tcp_pcb *)kf->socket_id, &ip, ntohs(addr->sin_port), tcp_connected_callback);
            regs->eax = 0xFFFFFFFF; return;
        }
        if (kf->is_dirty == 1) { kf->offset = 0; kf->is_dirty = 0; regs->eax = 0; } 
        else if (kf->is_dirty == -1) { kf->offset = 0; kf->is_dirty = 0; regs->eax = -1; } 
        else { regs->eax = 0xFFFFFFFF; }
    } else { regs->eax = -1; }
}
...
static void sys_send(struct syscall_registers *regs) {
    ...
    if (kf->type == FILE_TYPE_SOCKET) {
        struct pbuf *p = pbuf_alloc(PBUF_IP, size, PBUF_RAM); pbuf_take(p, data, size);
        regs->eax = raw_send((struct raw_pcb *)kf->socket_id, p); pbuf_free(p);
    } else if (kf->type == FILE_TYPE_SOCKET_TCP) {
        struct tcp_pcb *pcb = (struct tcp_pcb *)kf->socket_id;
        err_t err = tcp_write(pcb, data, size, TCP_WRITE_FLAG_COPY); tcp_output(pcb);
...
static void sys_recv(struct syscall_registers *regs) {
    ...
    if (kf->size > to_copy) { memmove(kf->buffer, kf->buffer + to_copy, kf->size - to_copy); kf->size -= to_copy; } 
    else { kf->size = 0; kf->is_dirty = (kf->is_dirty == -1) ? -1 : 0; }

    if (kf->type == FILE_TYPE_SOCKET_TCP && to_copy > 0) { tcp_recved((struct tcp_pcb *)kf->socket_id, to_copy); }
    regs->eax = to_copy;
}

static volatile int dns_state = 0; static ip_addr_t resolved_ip; static char kernel_hostname[256]; 
static void dns_callback(const char *name, const ip_addr_t *ipaddr, void *callback_arg) {
    if (ipaddr != NULL) { resolved_ip = *ipaddr; dns_state = 2; } else { dns_state = 3; }
}

static void sys_gethostbyname(struct syscall_registers *regs) {
    if (safe_strlen((char*)regs->ebx, 256) < 0) { regs->eax = 0; return; }
    char* user_hostname = (char*)regs->ebx;
    if (dns_state == 2) { dns_state = 0; regs->eax = resolved_ip.addr; return; } 
    else if (dns_state == 3) { dns_state = 0; regs->eax = 0; return; }

    if (dns_state == 0) {
        strcpy(kernel_hostname, user_hostname);
        ip_addr_t addr;
        err_t err = dns_gethostbyname(kernel_hostname, &addr, dns_callback, NULL);
        if (err == ERR_OK) { regs->eax = addr.addr; return; } 
        else if (err == ERR_INPROGRESS) { dns_state = 1; regs->eax = 0xFFFFFFFF; return; } 
        else { regs->eax = 0; return; }
    }
    regs->eax = 0xFFFFFFFF; 
}

static void sys_scancode_to_char(struct syscall_registers *regs) { regs->eax = (char)scancode_to_char((uint8_t)regs->ebx); }
...
static void sys_get_rtc(struct syscall_registers *regs) {
    if (!validate_user_ptr((void*)regs->ebx, sizeof(struct time))) return;
    struct time *t_ptr = (struct time *)regs->ebx;
    if (t_ptr) *t_ptr = rtc_get_time();
}

static void sys_sigaction(struct syscall_registers *regs) {
    int sig = regs->ebx; uint32_t handler = regs->ecx;
    if (sig > 0 && sig < 32 && current_task) { current_task->signal_handlers[sig] = handler; regs->eax = 0; } else regs->eax = -1;
}
...
static void sys_uname(struct syscall_registers *regs) {
    if (!validate_user_ptr((void*)regs->ebx, 65 * 4)) { regs->eax = -1; return; }
    char* sysname = (char*)regs->ebx;
    char* release = sysname + 65;
    char* version = release + 65;
    char* machine = version + 65;
    
    strcpy(sysname, OS_NAME);
    strcpy(release, OS_RELEASE); 
