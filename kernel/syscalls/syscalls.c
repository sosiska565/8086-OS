...
static void sys_set_blur(struct syscall_registers *regs) { 
    if(current_task && current_task->window) current_task->window->blur = regs->ebx; 
    wm_refresh();
}
static void sys_win_redraw_char_at(struct syscall_registers *regs) {
    window_redraw_char_at((Window*)regs->ebx, (int)regs->ecx, (int)regs->edx);
}

static syscall_handler_t syscall_table[256] = {
    [0]  = sys_win_putc, [1]  = sys_exit, [2]  = sys_getch, [3]  = sys_gets,
    [4]  = sys_malloc, [5]  = sys_free,[6]  = sys_clear_screen, [7]  = sys_print_color,[8]  = sys_random, [9]  = sys_print_num, [10] = sys_read_file,
    [17] = sys_print_hex, [18] = sys_get_file_size,[19] = sys_wait_scancode,[20] = sys_write_file, [21] = sys_scancode_to_char, [22] = sys_get_screen_w,
    [23] = sys_get_screen_h, [24] = sys_vprintf, [25] = sys_draw_rect,
    [26] = sys_get_screen_w, [27] = sys_get_screen_h, [30] = sys_create_win,
    [31] = sys_close_win,[32] = sys_exit, [33] = sys_kbd_flush, [34] = sys_wait_process,
    [35] = sys_win_print, [36] = sys_sleep, [37] = sys_fork, [38] = sys_kill,
    [39] = sys_win_render, [40] = sys_win_redraw,[41] = sys_win_draw_char,
    [42] = sys_system_cmd, [43] = sys_get_sys_info, [44] = sys_get_task_list,
    [45] = sys_reload_config, [46] = sys_set_blur, [47] = sys_win_redraw_char_at
};
