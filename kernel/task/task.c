...
void init_tasking() {
    ...
    if (!ktask) panic("Failed to allocate kernel task!");
...
int create_process(void (*entry)(int, char**), int argc, char **argv, char *name, struct page_directory_t *pd) {
    ...
    if (!new_task) {
        printf("[SYSTEM] Out of memory: Cannot create process struct!\n");
        return -1;
    }
    ...
    if (!stack) {
        printf("[SYSTEM] Out of memory: Cannot create process stack!\n");
        kfree(new_task);
        return -1;
    }
