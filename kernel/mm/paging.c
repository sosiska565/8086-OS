...
void init_paging() {
    klog("[PAGING] Initializing Page Directory...");
    ...
    char msg[128] = "[PAGING] Mapping VESA LFB at ";
    char hex[16];
    itoa(vesa_addr, hex, 16);
    strcat(msg, hex);
    strcat(msg, " (4MB Huge Pages)");
    klog(msg);
    ...
    _enablePaging();
    klog("[PAGING] CPU Paging Flag Enabled!");
}
...
page_directory_t* clone_page_directory() {
    ...
    for(int i = 0; i < 1024; i++) { dir->entries[i] = kernel_dir->entries[i]; }
    return dir;
}
